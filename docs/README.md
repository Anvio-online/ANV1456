...
#### Phase 2 — planned 2026-08-08, **everything except `/case-studies` built 2026-08-10**
...
| [case-studies-spec.md](specs/case-studies-spec.md) | `/case-studies` + `/case-studies/[slug]`                                 | 2    | Not built — blocked on **the Stratseek agreement**. The one page in this plan not shipped                                           |
| [service-leaf-spec.md](specs/service-leaf-spec.md) | 4 leaves — 3 Automate, 1 Build                                           | 3    | **2 of 4 built** — `whatsapp-automation`, `ai-chatbot-development`, on provisional slugs. Remaining 2 blocked on keyword validation |
| [industries-spec.md](specs/industries-spec.md)     | `/industries` + `/industries/ecommerce` + `/industries/accounting-firms` | 4    | **Built, in full**                                                                                                                  |
| [tools-spec.md](specs/tools-spec.md)               | `/tools/automation-roi-calculator`                                       | 4    | **Built**                                                                                                                           |
| [guides-spec.md](specs/guides-spec.md)             | `/guides` + 6–8 guides                                                   | 5    | **Built, in full — all 8 guides**                                                                                                   |

**Wave 0 is built** — the content adapter and six of seven Phase 2 section types, per [content-layer.md](engineering/content-layer.md). One deviation from its original plan is worth reading there: `next-mdx-remote/rsc`, not `@content-collections/mdx`, is what actually compiles guide and leaf bodies — the originally planned package renders client-side, which would have put that content outside the server-rendered HTML this site's SEO position depends on. `caseStudyBody` is the one section type still unbuilt, blocked with the case studies. Authoring model: [ADR-0006](engineering/adr/0006-content-page-authoring-model.md).

**Two real bugs surfaced and were fixed during the build, recorded rather than silently patched.** Both shipped leaves linked to an unbuilt leaf (`ai-agent-development`) in their `relatedLinks` and once inline in body copy — a live 404 waiting to happen, fixed by repointing at a real page. Separately, the `<Comparison>` MDX whitelist component crashes the build (array-literal JSX props arrive `undefined` under `next-mdx-remote/rsc`) — its one usage was rewritten as a plain markdown table; the component stays registered but broken until someone fixes it. Both are detailed in [content-layer.md](engineering/content-layer.md) and [phase-2-plan.md](specs/phase-2-plan.md) §7.

**No Phase 2 page carries a Tier 1 signature scene.** Deliberate, and held in every built page — [phase-2-plan.md](specs/phase-2-plan.md) §4, mapped in [motion-system.md](system/motion-system.md) §8a.
...
| Home's six industry tiles                                                | `/industries`                                               | Two of six (Ecommerce, Accounting & Finance) now link to real pages; four stay plain until they have one — shared source: `lib/content/industry-tiles.ts`                            |
| `footer.tsx`'s 12 service leaf links, `Products`, `Case Studies`, `Blog` | invented URLs, none built                                   | All four columns now real: Services (hub + 3 pillars + 2 leaves), Company, Resources (Guides, Industries, ROI calculator), Legal. Only `Case Studies`/`Products`/`Blog` still absent |
| `nav.tsx`'s `Case Studies`, `Blog`                                       | unbuilt pages                                               | Dropped; `Services` is the three-column mega-menu, with the 2 shipped leaves linked from Automate's column                                                                           |
...
**2a. Resolved — mobile has a real nav now.** `components/layout/mobile-nav.tsx` adds the `md:hidden` full-screen overlay §6.5 calls for: staggered links, body scroll locked, closes on Escape/link click/resize past `md`. Now also links Guides and Industries.

**3. Resolved — `footer.tsx` now has all four columns** [design-system.md](system/design-system.md) §6.6 specifies — Services / Company / Resources / Legal. Resources links Guides, Industries, and the ROI calculator; it does not yet link Case Studies, since that page doesn't exist.
...
**5. Resolved — `sitemap.ts` now generates itself.** Previously a literal list; now builds static routes from a literal array and content-driven routes (service leaves, guides, industries) from `contentRepository.list()`/`slugs()` — a new MDX file publishes itself here without a code change, which was the whole point of building a content adapter. `/case-studies` stays out until it has real entries.

**6. Found and fixed during the build — two links to an unbuilt leaf.** Both shipped leaves (`whatsapp-automation`, `ai-chatbot-development`) had a `relatedLinks` entry and, in the chatbot leaf, one inline body link pointing at `/services/automate/ai-agent-development` — a leaf that was never built, which would have 404'd for any visitor who clicked it. This shipped in an earlier commit and was only caught while auditing links before the above-the-floor build. Fixed: the frontmatter entries were removed, and the inline reference now points at the `ai-agent-vs-chatbot` guide instead.
...
- [ ] **Read the Stratseek agreement** for client-naming and attribution clauses — blocks the featured-work section on Home, and blocks `/case-studies`, the one page left in Phase 2
...
- [ ] Three insight posts, or cut the Home insights section — eight guides now exist and could seed it, but Home's own `insights` section instance was never added; a small separate step, not done in this build
...
- [ ] Decide who monitors `hello@anvio.online` and where form submissions notify — genuinely urgent now: `/tools/automation-roi-calculator` routes to `/contact`, and `/privacy` names this address as the deletion/access-request contact. Two live surfaces depend on it now, not zero
...
Full context in [phase-2-plan.md](specs/phase-2-plan.md) §7. Status as of 2026-08-10.
...
- [ ] **Read the Stratseek agreement** — the only remaining blocker in the whole plan. Blocks `/case-studies` and `/case-studies/[slug]` entirely, and `/projects`' two withheld client cards (Baladi, Epicerma)
- [ ] **Keyword-validate the four leaf slugs** in Ahrefs/Semrush and Search Console — the two shipped leaves (`whatsapp-automation`, `ai-chatbot-development`) launched on provisional slugs without this research running first; validate before building the remaining two, and be aware a rename to a shipped slug now costs a real redirect
...
- [x] ~~**A real author name**~~ — resolved as an upgrade path, not a blocker: all eight guides ship at the role-byline fallback (`"Anvio's founding engineer"`), no name. Swapping in a real name later is a frontmatter string change
...
- [ ] **Fix `<Comparison>`.** Found shipping `n8n-vs-zapier-vs-make` — the component crashes the production build (array-literal JSX props arrive `undefined` under `next-mdx-remote/rsc`). Its one usage was rewritten as a plain markdown table; the component stays in the whitelist, registered and broken, until someone diagnoses the actual cause
