...
| **Nobody owns `hello@anvio.online`**           | Live on `/privacy` and the ROI calculator's CTA. Inbound is arriving somewhere nobody is watching                   |
| **No published Lighthouse/CWV score**          | `/services/grow` claims performance; `/projects`' anvio.online card ships without numbers rather than invented ones |
| **No analytics wired at all**                  | There is no traffic data. Any "performance" claim about the site is currently unmeasurable                          |
