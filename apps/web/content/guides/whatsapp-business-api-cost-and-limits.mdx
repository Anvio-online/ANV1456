...
faq:
  - question: 'How is the WhatsApp Business API different from the WhatsApp Business app?'
    answer: 'The app is built for a person to use by hand and has no way for software to read or send messages. The Business API is a programmatic interface that lets software send and receive WhatsApp messages, which is what makes any automation possible. Moving from one to the other is a real setup step, not a settings toggle.'
  - question: 'What does WhatsApp charge once you are live?'
    answer: "Since 1 July 2025, Meta charges per delivered template message, categorized as marketing, utility, or authentication, with rates that vary by category and recipient country. Free-form service replies sent inside a customer's 24-hour window are not charged, and Meta includes 1,000 free service conversations per month per account."
  - question: 'What is the WhatsApp 24-hour rule?'
    answer: 'Once a customer messages you, you have a 24-hour window to send free-form replies about anything. After it closes you can only send a pre-approved template message. Anything that must reach a customer outside an active conversation — a later shipping update, a re-engagement message — has to be built as a template from the start.'
  - question: 'Do you need a Business Solution Provider (BSP) for WhatsApp automation?'
    answer: "You cannot connect to the Business API directly as an individual developer — access is through Meta's Cloud API or an approved BSP. Most small businesses use a BSP because it handles the sending infrastructure and support. A BSP typically adds a small per-message markup on top of Meta's own rates."
  - question: 'How long does WhatsApp Business API setup take?'
    answer: 'Business verification takes a few days to two weeks, BSP or Cloud API setup a few days, template approval minutes to a couple of days each, and the automation build itself typically 2–4 weeks depending on scope. Run verification in parallel with the build rather than on the critical path.'
relatedLinks:
  - label: 'WhatsApp automation'
    href: '/services/automate/whatsapp-automation'
...
The WhatsApp Business API costs money in two separate places — a one-time or ongoing build cost, and Meta's own per-message fees once you're live — and the second one is the part most vendors gloss over until it shows up on an invoice. This guide covers both, plus the structural limit that shapes every WhatsApp automation: the 24-hour window.
...
This is the part that catches people off guard, because it's billed by Meta directly, separately from whatever you paid to build the automation, and the rate structure has changed more than once as Meta has evolved its pricing model. Since 1 July 2025, WhatsApp charges **per delivered template message**, not per conversation. Templates are categorized as **marketing**, **utility** (like order updates), or **authentication** (login codes), with a different rate per category and per recipient country. **Service messages** — free-form replies you send inside a customer's 24-hour window — are **not charged at all**, and Meta additionally includes 1,000 free service conversations per month per account.

The practical shape of this: if your automation only ever replies to customers who messaged first, and does so within 24 hours, Meta's message fees can be zero. Costs start when you send **templates** — a shipping update two days later, a re-engagement message to a quiet lead — because those go out to a category and country rate.

We're deliberately not quoting the specific per-message numbers here, for a reason worth stating plainly: Meta's rates change periodically (this model itself replaced the previous per-conversation one), and a specific figure in a guide like this becomes stale faster than the guide gets updated, which would make it actively misleading rather than just outdated. The categories and the free-service-window rule are stable; the numbers attached to the template categories are not, and the responsible thing is to confirm the current rate directly — either from Meta's own published pricing for your market, or from whoever builds your automation, before you launch, not after. Note also that a Business Solution Provider usually adds its own small per-message markup on top of Meta's rate.
...
The build here is squarely mid-scope: one intent (order capture), one integration (the CRM), and two message types (free-form confirmation, template shipping update). Under the current per-message model, the ~40 daily order confirmations cost nothing — they're free-form service replies inside the customer's own 24-hour window. The only Meta message fee is on the **template** shipping updates sent a day or two later, outside that window: one utility-category template per order, at that category's rate for your market. Against 2–3 hours of daily manual labor removed, that ongoing cost is, for the overwhelming majority of businesses at this volume, the smaller number by a wide margin — but "for most businesses" isn't the same as "for yours," which is why this is worth actually running the numbers on at current rates rather than assuming.
...
**How is the WhatsApp Business API different from the WhatsApp Business app?** The app is built for a person to use by hand and has no way for software to read or send messages. The Business API is a programmatic interface that lets software send and receive WhatsApp messages, which is what makes any automation possible. Moving from one to the other is a real setup step, not a settings toggle.

**What does WhatsApp charge once you're live?** Since 1 July 2025, Meta charges per delivered template message, categorized as marketing, utility, or authentication, with rates that vary by category and recipient country. Free-form service replies sent inside a customer's 24-hour window aren't charged, and Meta includes 1,000 free service conversations per month per account.

**What is the WhatsApp 24-hour rule?** Once a customer messages you, you have a 24-hour window to send free-form replies about anything. After it closes you can only send a pre-approved template message. Anything that must reach a customer outside an active conversation — a later shipping update, a re-engagement message — has to be built as a template from the start.

**Do you need a Business Solution Provider (BSP)?** You can't connect to the Business API directly as an individual developer — access is through Meta's Cloud API or an approved BSP. Most small businesses use a BSP because it handles the sending infrastructure and support. A BSP typically adds a small per-message markup on top of Meta's own rates.

**How long does setup take?** Business verification takes a few days to two weeks, BSP or Cloud API setup a few days, template approval minutes to a couple of days each, and the automation build itself typically 2–4 weeks depending on scope. Run verification in parallel with the build rather than on the critical path.

## The honest summary

Budget for three things, not one: the verification and build cost, Meta's ongoing per-template-message fees (confirmed at current rates for your specific market, not estimated from an old guide or a competitor's number — and remember free-form replies inside the 24-hour window aren't charged), and template approval time built explicitly into your launch schedule rather than assumed away. Design around the 24-hour window from the start rather than discovering it after a real customer's window has already closed. Do those three things and the WhatsApp Business API is one of the most reliably useful automations a small business can build — most of what goes wrong with it comes from skipping the planning, not from the technology itself.
