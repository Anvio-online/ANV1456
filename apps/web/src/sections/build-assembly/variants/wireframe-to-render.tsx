...
import { BuildAssemblyStaticList } from '../build-assembly-static-list'
 ...
 * as process/variants/horizontal-pin.tsx and
 * workflow-graph/variants/live.tsx — see either's docstring for why
 * the dynamic import has to live in a Client Component here rather
 * than in build-assembly/index.tsx.
 */
const BuildAssemblyScene = dynamic(
  () => import('@/scenes/build-assembly/build-assembly').then((m) => m.BuildAssemblyScene),
  { ssr: false },
)

export function WireframeToRender({
  eyebrow,
  heading,
  body,
  passes,
  headingTag,
}: BuildAssemblyProps & { headingTag: HeadingTag }) {
  const HeadingTagEl = headingTag
  const containerRef = useRef<HTMLDivElement>(null)

  // Gate 1: desktop + motion-allowed only — motion-system.md §7.3's
  // reduced-motion/mobile fallback is the static list, same as
  // Process and the Workflow Graph.
  ...
  return (
    <div ref={containerRef}>
      ...
      {showScene ? (
        <BuildAssemblyScene passes={passes} />
      ) : (
        <BuildAssemblyStaticList passes={passes} />
      )}
    </div>
  )
}
